...
SELECT listings.property_type, COUNT(reviews.id), MAX(reviews.date_reviewed) FROM listings INNER JOIN reviews ON reviews.listing_id = listings.id GROUP BY listings.property_type;